...
function sse_admin_enqueue_assets( $hook ) {
    ...
    if ( false === strpos( $hook, 'sse-encyclopedia' ) && ( ! $screen || 'sse_article' !== $screen->post_type ) ) return;
    wp_enqueue_style( 'dashicons' );
    wp_add_inline_style( 'dashicons', '.sse-admin-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;max-width:1180px}.sse-admin-card{background:#fff;border:1px solid #dcdcde;padding:18px;box-shadow:0 1px 2px rgba(0,0,0,.04)}.sse-admin-card h2,.sse-admin-card h3{margin-top:0}.sse-admin-stat{font-size:30px;font-weight:700;color:#8f1822}.sse-admin-muted{color:#646970}.sse-admin-bilingual{display:grid;grid-template-columns:1fr 1fr;gap:18px}.sse-admin-field{display:grid;gap:5px;margin:0 0 14px}.sse-admin-field input,.sse-admin-field select,.sse-admin-field textarea{width:100%}.sse-admin-field textarea{min-height:90px}.sse-admin-status{display:inline-block;padding:3px 8px;border-radius:12px;background:#f0f0f1}.sse-admin-warning{border-left:4px solid #dba617;padding:10px 12px;background:#fff8e5}@media(max-width:900px){.sse-admin-grid,.sse-admin-bilingual{grid-template-columns:1fr}}' );
}
